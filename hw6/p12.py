...
# my hypothesis [theta, s, i, alpha]
H = []

# Err return [one_error, theta, s, i, alpha, magic_err]
...
for alg in H:
    data = X_train[:, alg[2]]
    x_list = np.asarray(map(lambda x: alg[-1]*h(x, alg[0], alg[1]), data))
    X.append(x_list)

G = np.sum(np.array(X), axis=0)
G = map(lambda x: sign(x), G)

print sum(G != np.asarray(y_train))/100
